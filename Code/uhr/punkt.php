<?php 
	...
	//nga fili wortanpassung.js marrim nr total te pikeve qe ka fituar useri te loja dhe kete numer do e fusim
	...
	if ($result[0]['total'] != 0) {
		$query = "update progress set uhr_tabelle_progress = ".$punkt." where kunde_ID = '".$_SESSION['kunde_ID']."'";
		$st = $dbh->prepare($query);
		$st->execute();
		echo "table updatet";
	}else{
		$st2 = $dbh->prepare("insert into progress (kunde_ID, uhr_tabelle_progress) values ('".$_SESSION['kunde_ID']."', '".$punkt."')");
		$st2->execute();
	...
	$total = intval($r[0]['bildwort_progress']) + intval($r[0]['uhr_tabelle_progress']) + intval($r[0]['rede_satzerg_progress']) + intval($r[0]['satzerg_progress']) + intval($r[0]['flashcard_progress']) + intval($r[0]['wortanpassung_progress']);
