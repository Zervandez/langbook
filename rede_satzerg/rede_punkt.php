<?php 
	...
	if ($result[0]['total'] != 0) {
		$query = "update progress set rede_satzerg_progress = ".$punkt." where kunde_ID = '".$_SESSION['kunde_ID']."'";
		$st = $dbh->prepare($query);
		$st->execute();
		echo "table updatet";
	}else{
		$st2 = $dbh->prepare("insert into progress (kunde_ID, rede_satzerg_progress) values ('".$_SESSION['kunde_ID']."', '".$punkt."')");
		$st2->execute();
